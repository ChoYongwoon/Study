...
long long int recursiveCheck(int front, int back){
    if(front == back){return height[front];} // 기저, size가 1이면 높이를 리턴
    
    int midpoint = (front + back)/2; // front와 back의 중간지점
    long long int left = recursiveCheck(front, midpoint); // left segment 재귀 수행
    long long int right = recursiveCheck(midpoint+1, back); // right segment 재귀 수행
    long long int res = max(left, right); // 둘 중 큰 값을 결과에 저장

    // 왼쪽 오른쪽에 걸쳐있는 경우 계산
    int mid_l = midpoint; int mid_r = midpoint+1; // 왼쪽의 마지막 요소와 오른쪽의 첫 요소
    long long int h = min(height[mid_l], height[mid_r]); // 직사각형 높이
...
int main() {
    while(true){
        ...
        long long int res = recursiveCheck(0, n-1);
        cout << res << endl;
    }
}